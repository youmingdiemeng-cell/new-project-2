// scripts/core/phone-shell.js
// Phone shell rendering, app navigation, and device-aware layout.
...
function hasUnreadMessages() {
  return MESSAGE_DATA.contacts.some((contact) => {
    const visibleMessages = contact.messages.filter(
      (message) => message.phase <= GameState.gamePhase && message.from !== 'me'
    );
    if (visibleMessages.length === 0) return false;

    const seenCount = typeof GameState.readChats[contact.id] === 'number'
      ? GameState.readChats[contact.id]
      : (GameState.readChats[contact.id] ? visibleMessages.length : 0);

    return visibleMessages.length > seenCount;
  });
}
...
function applyPhoneViewportLayout() {
  ...
  if (isMobile) {
    ...
    const shellScale = Math.max(0.82, window.innerWidth / PHONE_BASE_WIDTH);
...
function isKeyboardEligibleField(element) {
  ...
  return element.matches(
    'input:not([type="checkbox"]):not([type="radio"]):not([type="button"]):not([type="submit"]):not([type="range"]):not([type="color"]):not([type="file"]), textarea, select'
  );
}
...
function updateMobileKeyboardAnchor() {
  if (!isMobileGameDevice() || !activeMobileInput || !activeKeyboardAnchor || !window.visualViewport) {
    return;
  }
...
function bindMobileKeyboardHandling() {
  ...
  document.addEventListener('focusin', (event) => {
    ...
    window.setTimeout(updateMobileKeyboardAnchor, 220);
  }, true);

  document.addEventListener('focusout', (event) => {
    if (!activeMobileInput || event.target !== activeMobileInput) return;
    window.setTimeout(() => clearMobileInputAnchor(false), 120);
  }, true);
        ...
        <div class="phone-screen">
          <div class="status-bar">
            <div class="status-left">
              <span class="status-time">${typeof getGameTimeString === 'function' ? getGameTimeString() : '23:47'}</span>
            </div>
...
function renderHomeScreen() {
  ...
  const appIconsHtml = APPS
    .filter((app) => !DOCK_APPS.includes(app.id))
    .map((app) => `
      <div class="app-icon" onclick="openApp('${app.id}')">
        <div class="app-icon-inner" style="background: ${app.gradient}">${app.icon}</div>
        <span class="app-label">${app.label}</span>
      </div>
    `)
    .join('');

  const dockHtml = DOCK_APPS.map((id) => {
    const app = APPS.find((item) => item.id === id);
    const badgeHtml = id === 'messages' && hasUnreadMessages() ? '<span class="badge">1</span>' : '';
    return `
      <div class="app-icon" onclick="openApp('${app.id}')">
        <div class="app-icon-inner" style="background: ${app.gradient}">
          ${app.icon}
          ${badgeHtml}
        </div>
        <span class="app-label">${app.label}</span>
  ...
  screen.innerHTML = `
    <div class="home-screen">
      <div class="home-header">
        ...
        <span id="homeTimeDisplay" style="color:rgba(255,255,255,0.3);font-size:11px;">${typeof getGameTimeString === 'function' ? `深夜 ${getGameTimeString()}` : '深夜 23:47'}</span>
      </div>
      <div class="app-grid">
        ${appIconsHtml}
      </div>
...
function openApp(appId) {
  if (typeof stopAllRadioAudio === 'function') {
    stopAllRadioAudio();
  }

  const screen = document.getElementById('screenContent');
  if (screen) {
    screen.classList.remove('home-active');
  }
